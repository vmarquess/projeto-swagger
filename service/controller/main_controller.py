...
from service.service.main_service import BinaryService
...
class MainService(Resource):
    ...
    def post(self) -> dict:
        try:
            ...
            main_service = BinaryService()
            resp = main_service.executar_rest(dados_request)
            response = objResponse.send_success(data=resp, messages=mensagens.SUCESSO_SERVICO, status=codeHttp.SUCCESS_200)
